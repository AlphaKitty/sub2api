export default {
    ...
    groups: {
      ...
      form: {
        ...
        description: '描述',
        platform: '平台',
        displayPlatform: '展示平台（可选）',
        displayPlatformPlaceholder: '不覆盖（按真实平台展示）',
        rateMultiplier: '费率倍数',
        status: '状态',
        exclusive: '专属分组',
        nameLabel: '分组名称',
      ...
      displayPlatformTooltip: '用户侧图标/徽章按展示平台渲染，路由与计费仍按真实平台',
      displaysAs: '展示为 {platform}',
      displayPlatformHint: '仅影响用户侧看到的图标/徽章品牌，不影响路由、计费与账号池',
      noGroupsYet: '暂无分组',
      ...
      healthPolicyThreshold: '连续失败阈值',
      healthPolicyOnFailure: '失败处置',
      healthPolicyActionDisable: '关闭调度 (schedulable=false)',
      healthPolicyActionNone: '仅记录，不处置',
      healthPolicyRecover: '成功时恢复限流/临时状态',
      healthPolicyReenable: '成功时重新启用已关闭账号',
      healthPolicyRunNow: '立即执行',
      healthPolicyRunning: '执行中…',
      healthPolicyDelete: '删除策略',
      healthPolicyHistory: '最近执行记录',
      healthPolicyNoRuns: '暂无执行记录',
      healthPolicyViewItems: '查看明细',
      healthPolicyNoItems: '无账号明细',
      healthPolicyAccount: '账号',
      healthPolicyAction: '动作',
      healthPolicyRunSummary: '共 {total} · 成功 {ok} · 失败 {fail} · 处置 {action}',
      healthPolicyModelRequired: '请填写测试模型',
      healthPolicyLoadFailed: '加载健康策略失败',
      healthPolicySaved: '健康策略已保存',
      healthPolicySaveFailed: '保存健康策略失败',
      healthPolicyRunDone: '巡检已完成',
      healthPolicyRunFailed: '执行巡检失败',
      healthPolicyDeleted: '健康策略已删除',
      healthPolicyDeleteFailed: '删除健康策略失败',
